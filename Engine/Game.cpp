...
Game::Game( MainWindow& wnd )
	...
	gfx( wnd ),
    rnd(),
    board(gfx, board_width, board_height),
	snake({ board_width / 2,board_height / 2 + board.getBoardScreenDistance() }),
	fruit( rnd.get_rand_numb(board.getBoardScreenDistance(), board.getBoardScreenDistance() + board.getGridWidth() -1), 
		rnd.get_rand_numb(board.getBoardScreenDistance() + 1, board.getBoardScreenDistance() + board.getGridHeight()) -1)
{
}
...
void Game::UpdateModel()
{
	if (!game_over) {
		...
		if (speed_counter == 0) {
			...
			if (snake.move_backwards(next_dir)) next_dir = snake.get_direction(); // doesn't allow player to move backwards - keeps previous direction
			snake.change_direction(next_dir);

			for (auto& i : obstacles) 
				if(snake.check_obstacle_collision(next_dir, i.get_loc())) game_over = true;

			if (game_over || snake.check_board_collision(board, next_dir) || snake.check_body_collision(next_dir)) game_over = true;

			else
			{
				snake.MoveByDir(next_dir);

				if (snake.is_fruit_eaten(fruit.get_loc())) {
					++score;
					if (speed > 0 && score % 2 == 0) {
						--speed;
					}
					snake.Grow(Color(0, rnd.get_rand_numb(0, 255), 0));
					fruit.reset_position(rnd, snake, board, obstacles);
				}
			}
		}
		++speed_counter;
		if (speed_counter == speed) {
			speed_counter = 0;
			++obstacles_timer;
			if (obstacles_timer == 10) {
				obstacles_timer = 0;
				Obstacle new_b = new_b.new_obstacle(rnd, snake, fruit.get_loc(), board);
				obstacles.push_back(new_b);
			}
		}
	}
}

void Game::draw_game_over(int x, int y)
{
...
{
	fruit.Draw(board);
	for (auto& i : obstacles) i.Draw(board);
	board.draw(gfx);
    snake.Draw(board);
    
	gfx.draw_filled_rect(10, 10, 10 * (16 - speed), 10, Colors::Blue); //speed
	if (game_over) draw_game_over(358, 268);
}
